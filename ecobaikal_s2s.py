...
from oper_tools import check_meteo, short_corr, graphShort, readShort
...
def ecorun(date_start, date_end, meteo_path, hydro_path, baspath, exepath, exename, ens_flag, dir_CT, dir_out,
    ...
    if isinstance(date_start, Timestamp):
        print('date start not date')
        date_start = pd.to_datetime(date_start)
    if not isinstance(date_end, Timestamp):
        print('date end not date')
...
def ecocycle(dates, lead, params):
    ...
    for date in dates:
        ...
        params['dir_out'] = 'd:\\EcoBaikal\\Archive\\004\\CT\\'
        print('ERA5Land', model_start, model_end, params['meteo_path'], params['dir_out'])
        ecorun(model_start, model_end, **params)

        # сохраняем датафрейм контрольной точки для последующего сшивания с прогнозами
        s2s_df = readShort(params['dir_out'] + model_end.strftime("%Y%m%d")  + '\\' + params['source_name'])
        s2s_df.drop(columns='lag', inplace=True)
        s2s_df['ens'] = '99'
        ...
        old_meteo = params['meteo_path']
        for s in range(0, 25):
            # params['meteo_path'] = params['meteo_path'] + '\\S2S\\' + date.strftime("%y%m%d") + '\\' + str(s) + '\\'
            params['meteo_path'] = 'D:\\Data\\S2S\\BAS\\' + date.strftime("%Y%m%d") + '\\' + str(s) + '\\'
            model_start = date
            ...
            res['ens'] = s
            s2s_df = pd.concat([s2s_df, res], ignore_index=True)
        params['meteo_path'] = old_meteo
        s2s_df.to_csv(params['dir_out'] + '\\' + model_end.strftime("%Y%m%d") + '\\' + 's2s_' + model_end.strftime("%Y%m%d") + '.txt')
        print(s2s_df.head())
...
if __name__ == "__main__":
    params = read_params(os.path.join(sets.MODEL_DIR, 'baikal_s2s.txt'))
    dates = []
    # for y in range(int(params['year_start']), int(params['year_end']) + 1):
    dates = pd.date_range(start=pd.to_datetime('2001-05-01'), end=pd.to_datetime('2025-10-01'), freq='MS')
